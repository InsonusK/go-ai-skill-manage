...
| Local / GitHub | LocalSource, RepositoryFetcher, GitCloner, ArchiveFetcher |
...
| SyncService | Orchestrator | [SyncService.Run](../../internal/domain/services/sync.go) — Координирует стадии одного запуска | SourceProvider, Detector, Expander, Planner, PlanWriter | [условия](../../internal/domain/services/usecases.md), [сценарии](../../internal/domain/services/TESTS.md) |
...
| SkillCatalog | Service | [Catalog.Add / Owner](../../internal/domain/services/discovery/catalog.go) — Разрешает коллизии скилов | модели каталога | [условия](../../internal/command/usecases.md), [сценарии](../../internal/command/TESTS.md) |
...
| OutputLayout | Function | [BuildLayout](../../internal/domain/services/planning/layout.go) — Назначает выходные пути | Catalog, fs.FS для внешних вложений | [условия](../../internal/domain/services/planning/usecases.md), [сценарии](../../internal/domain/services/planning/TESTS.md) |
...
| SyncPlanner | Service | [Planner.Plan](../../internal/domain/services/planning/planner.go) — Определяет операции обновления цели | StateReader, DocumentCodec, OutputLayout, преобразования | [условия](../../internal/domain/services/planning/usecases.md), [сценарии](../../internal/domain/services/planning/TESTS.md) |
...
- [model](../../internal/domain/model/model.go): Request → Repository/Skill/Link → TargetPlan → Result; ошибки Issue/Issues.
- [interfaces](../../internal/domain/interfaces/ports.go): SourceProvider, DocumentCodec, StateReader, PlanWriter. Получатель порта определяет необходимую роль.
...
2. SyncService получает источники, регистрирует cleanup, вызывает SourceSelector и SkillCatalog.
3. RelationExpander обходит Markdown; LinkResolver находит владельца или внешнее вложение.
4. SyncPlanner строит все TargetPlan, читая StateReader. OutputLayout и преобразования готовят окончательные байты.
5. Dry-run возвращает планы. Обычный запуск передаёт планы PlanApplier.
6. ResultFormatter выводит результат; cleanup источников выполняется и при ошибках.

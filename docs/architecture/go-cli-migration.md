...
В каждом тестируемом пакете: `features/*.feature` со сценариями,
`test/runner_test.go` с одним `TestFeatures`, `test/*_steps_test.go` по
понятиям. Названия подкаталогов `services` обозначают самостоятельные
предметные области; внутри каждого файла — одна ответственность.
